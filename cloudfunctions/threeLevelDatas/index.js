...
async function getInfoData(company, step) {
...
async function getInfoDatas(company) {
  let count = await getInfoCount(company)
  let datas = []
  for (var i = 0; i < count; i += 100) {
    console.log("info i = ", i)
    datas = datas.concat(await getInfoData(company, datas.length))
  }
